...
class SettingsActivity : SimpleActivity() {
    ...
    private fun setupWallCalendar() = binding.apply {
        settingsWallStartMode.isChecked = config.wallStartMode
        ...
        settingsWallAddress.text = if (addresses.isEmpty()) {
            "http://127.0.0.1:${WebCalendarService.PORT}"
        } else {
            addresses.joinToString("\n") { "http://$it:${WebCalendarService.PORT}" }
        }
        settingsWallLocalWebInterfaceHolder.setOnClickListener {
        ...
        settingsWallAddressHolder.setOnClickListener {
            val address = settingsWallAddress.text.toString()
