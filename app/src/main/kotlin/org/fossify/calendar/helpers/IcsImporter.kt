...
class IcsImporter(private val context: Context, private val errorHandler: ((Exception) -> Unit)? = null) {
    constructor(activity: SimpleActivity) : this(activity, { activity.showErrorToast(it) })
    ...
    fun importEvents(
        ...
        try {
            ...
            val existingEvents = context.eventsDB.getEventsOrTasksWithImportIds()
                        ...
                        if (eventToUpdate == null) {
                            ...
                            if (isSequence) {
                                if (curRecurrenceDayCode.isEmpty()) {
                                    ...
                                    val parentEvent =
                                        context.eventsDB.getEventWithImportId(event.importId)
            ...
            eventsHelper.insertEvents(
                tasks as ArrayList<Event>,
                addToCalDAV = false,
                allowReadOnlyCalendar = true
            )
            eventsHelper.insertEvents(
            ...
            tasks.filter { it.isTaskCompleted() }.forEach {
                context.updateTaskCompletion(it, completed = true)
            }
        } catch (e: Exception) {
    ...
    private fun tryAddCategories(categories: String) {
        ...
        curCalendarId = if (calendarId == -1L) {
            val newTypeColor = if (curCategoryColor == -2) {
                context.resources.getColor(org.fossify.commons.R.color.color_primary)
            } else {
                curCategoryColor
            }
