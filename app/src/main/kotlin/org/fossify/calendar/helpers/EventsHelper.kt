...
class EventsHelper(val context: Context) {
    ...
    fun insertEvent(
        ...
        allowReadOnlyCalendar: Boolean = false,
        callback: ((id: Long) -> Unit)? = null
    ) {
        if (!allowReadOnlyCalendar && isReadOnlyCalendar(event.calendarId)) {
            callback?.invoke(0)
            return
        }
    ...
    fun deleteEvent(
        ...
        deleteEvents(arrayListOf(id), deleteFromCalDAV, updateWidgets, allowReadOnlyCalendar)
    }
    ...
    fun deleteEvents(
        ...
        val idsToDelete = if (allowReadOnlyCalendar) {
            ids
        } else {
            ids.filter { id ->
                val event = eventsDB.getEventOrTaskWithId(id)
                event == null || !isReadOnlyCalendar(event.calendarId)
            }.toMutableList()
        }
        ...
        idsToDelete.chunked(CHOPPED_LIST_DEFAULT_SIZE).forEach {
            ...
            deleteChildEvents(it as MutableList<Long>, deleteFromCalDAV, updateWidgets, allowReadOnlyCalendar)
            if (updateWidgets) context.updateWidgets()
        }
    }
    ...
    private fun deleteChildEvents(
        ...
        if (childIds.isNotEmpty()) {
            deleteEvents(childIds, deleteFromCalDAV, updateWidgets, allowReadOnlyCalendar)
        }
    }

    private fun isReadOnlyCalendar(calendarId: Long): Boolean =
        calendarsDB.getCalendarWithId(calendarId)?.webReadOnly == true
